...
export class AddUpdateDesignationMasterComponent {
  ...
  getDesiganationLevel() {   
    ...
    this.editFlag ? (this.designationForm.controls['designationLevelId'].setValue(this.editData.designationLevelId),this.getDesiganationType()) : '';
  }

  getDesiganationType() {  
    let lan=this.designationForm.value.lan; 
    let desigLevelId= this.designationForm.value.designationLevelId; 
    
    this.masterService.GetDesignationByLevelId(lan,desigLevelId).subscribe({
      next: ((res: any) => {
        if (res.statusCode == '200' && res.responseData.length) {
